## 임베디드 개발자 서민솔 입니다. 👋 
여러 프로젝트를 통해 **센서·카메라 기반 시스템**, **실시간 데이터 처리**,  
**AI Edge 디바이스**, **IoT 기기**, **Linux 기반 임베디드 환경**을 개발해왔습니다.

저는 단순히 코드를 작성하는 개발자가 아니라,  
**문제 정의 → 시스템 설계 → 구현 → 검증**까지  
전 과정을 책임지는 엔지니어를 목표로 하고 있습니다.

현장에서 실제로 사용할 수 있는 시스템,  
그리고 오랫동안 안정적으로 동작하는 시스템을 만드는 것이 저의 가장 큰 포부입니다.

## 🧰 Tech Stack

#### 📌 Languages  
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-007396?style=flat&logo=openjdk&logoColor=white)
![Verilog](https://img.shields.io/badge/Verilog-CC0000?style=flat&logo=verilog&logoColor=white)

![STM32](https://img.shields.io/badge/STM32-03234B?style=flat&logo=stmicroelectronics&logoColor=white)
![ARM](https://img.shields.io/badge/ARM-0091BD?style=flat&logo=arm&logoColor=white)
![RTOS](https://img.shields.io/badge/RTOS-0A72B1?style=flat&logo=freertos&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino%20Uno-00979D?style=flat&logo=arduino&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?style=flat&logo=raspberrypi&logoColor=white)
![Hailo](https://img.shields.io/badge/Hailo-333333?style=flat)

![Design Compiler](https://img.shields.io/badge/Design%20Compiler-3949AB?style=flat)
![ICC2](https://img.shields.io/badge/ICC2-6A1B9A?style=flat)
![Vivado](https://img.shields.io/badge/Vivado-FFCE00?style=flat&logo=xilinx&logoColor=black)

![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?style=flat&logo=ubuntu&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![Qt](https://img.shields.io/badge/Qt-41CD52?style=flat&logo=qt&logoColor=white)
![STM32CubeIDE](https://img.shields.io/badge/STM32CubeIDE-03234B?style=flat&logo=stmicroelectronics&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![GStreamer](https://img.shields.io/badge/GStreamer-3085C3?style=flat&logo=gstreamer&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat&logo=mysql&logoColor=white)

## 🚀 Skills / Expertise
![Firmware](https://img.shields.io/badge/Firmware-0A9396?style=flat)
![TCP/IP Socket](https://img.shields.io/badge/TCP%2FIP%20Socket-005F73?style=flat)
![Device Driver](https://img.shields.io/badge/Device%20Driver-001219?style=flat)
![BSP](https://img.shields.io/badge/BSP-CA6702?style=flat)
![IoT](https://img.shields.io/badge/IoT-94D2BD?style=flat)
- **Firmware Development**
- **TCP/IP Socket Programming**
- **Device Driver Development (Linux Kernel)**
- **BSP (Board Support Package) Customization**
- **IoT System Development**

## ⚙️ My Projects


#### [🚶 안전한 모터](https://link.url)  
RFID 사용자 인증과 IR 리모컨 제어를 RTOS 기반 멀티태스킹 구조로 통합해 인증된 사용자만 모터를 제어할 수 있는 임베디드 보안 모터 시스템입니다.

#### [🔐 보안 금고 서비스 [SSS: Security Safe Service]](https://link.url) 
TCP/IP 소켓 통신으로 금고(캐비넷) 단말과 서버를 연결해, 사용자 인증·상태 모니터링·원격 제어가 가능한 IoT 보안(캐비넷) 금고 서비스입니다.

#### [🏋️‍♂️ 자세어때 (AI Smart Gym)](https://link.url)
카메라 포즈 인식과 IMG·IMU 센서를 결합해, 스쿼트 등 운동 동작을 실시간으로 분류·채점하고 피로도·불균형까지 분석해주는 AI 기반 스마트 피트니스 시스템입니다.





